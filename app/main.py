...
from app.host_managed.internal_llm import InternalLLM
...
async def openai_node(state: LLMState) -> LLMState:
    try:
        server_params = StdioServerParameters(
            command="python",
            args=["-u", "app/servers/openai_server.py"],
...
async def llama_node(state: LLMState) -> LLMState:
    try:
        server_params = StdioServerParameters(
            command="python",
            args=["-u", "app/servers/llama_server.py"],
            env={"PYTHONPATH": os.getcwd()}
        )
        async with stdio_client(server_params) as (read_stream, write_stream):
            async with ClientSession(read_stream, write_stream) as session:
...
async def rag_node(state: LLMState) -> LLMState:
    try:
        server_params = StdioServerParameters(
            command="python",
            args=["-u", "app/servers/rag_mcp_server.py"],
            env={"PYTHONPATH": os.getcwd()}
        )
        async with stdio_client(server_params) as (read_stream, write_stream):
            async with ClientSession(read_stream, write_stream) as session:
...
async def crewai_node(state: LLMState) -> LLMState:
    try:
        server_params = StdioServerParameters(
            command="python",
            args=["-u", "app/servers/crewai_server.py"],
            env={"PYTHONPATH": os.getcwd()}
        )
        async with stdio_client(server_params) as (read_stream, write_stream):
            async with ClientSession(read_stream, write_stream) as session:
